...
- op "add_bot" creates an Automation bot with a DATA-CHANGE event and one or more process steps. Requires "table" (the event table/slice) + "name" (bot name) + "steps" (non-empty array). Optional: "eventName" (event display name; default auto "New event N"), "condition" (event runs only when true), "dataChangeType" (which changes fire the event — an ARRAY subset of ["Adds","Deletes","Updates"], e.g. ["Adds","Updates"]; omit for the default = all three), "bypassSecurity" (true/false). Each step runs a data action ("type":"run_a_data_action") in ONE of two modes: (a) EXISTING action — set "action" to the name of an existing action ON THE BOT'S TABLE (create it earlier with add_action); nothing else needed. (b) CUSTOM run-on-rows — set "custom":"run_action_on_rows" + "action" (the action to run) + "table" (Referenced Table) + "rows" (Referenced rows expression, the set of rows to run on). To use action types add-row/delete/set-value/grouped in a bot, create them via add_action first, then reference by name (mode a). Order: put add_action changes BEFORE the add_bot that references them.
